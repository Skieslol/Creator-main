...
const CheckT = require("../Creator_data/others/CT")
...
app.post("/fortnite/api/game/v2/profile/:accountId/client/EquipBattleRoyaleCustomization", async(req, res) => {
    ...
    switch (req.body.slotName) {
        ...
        case "Dance":
            if (req.body.indexWithinSlot == -1) {
                ...
                await Athena.updateOne({ id: req.params.accountId }, {
                    [req.body.slotName.toLowerCase()]: list
                })
            } else {
                if (req.body.itemToSlot == "") {
                    await Athena.updateOne({ id: req.params.accountId }, {
                        $set: {
                            [`${req.body.slotName.toLowerCase()}.${req.body.indexWithinSlot}`]: ""
                        }
                    })
                } else {
                    await Athena.updateOne({ id: req.params.accountId }, {
                        $set: {
                            [`${req.body.slotName.toLowerCase()}.${req.body.indexWithinSlot}`]: `${req.body.itemToSlot.split(":")[0]}:${req.body.itemToSlot.split(":")[1].toLowerCase()}`
                        }
                    })
                }
            }
        ...
        default:
            if (req.body.itemToSlot == "") {
                await Athena.updateOne({ id: req.params.accountId }, {
                    [req.body.slotName.toLowerCase()]: ""
                })
            } else {
                await Athena.updateOne({ id: req.params.accountId }, {
                    [req.body.slotName.toLowerCase()]: `${req.body.itemToSlot.split(":")[0]}:${req.body.itemToSlot.split(":")[1].toLowerCase()}`
                })
            }
            break;
    }
    ...
    if (req.body.variantUpdates ? req.body.variantUpdates.length != 0 : false) {
        await Athena.updateOne({ id: req.params.accountId }, {
            [`${req.body.slotName.toLowerCase()}variants`]: req.body.variantUpdates
        })
    }
...
app.post("/fortnite/api/game/v2/profile/:accountId/client/SetCosmeticLockerSlot", async(req, res) => {
    ...
    switch (req.body.category) {
        ...
        case "ItemWrap":
            if (req.body.slotIndex == -1) {
                ...
                await Athena.updateOne({ id: req.params.accountId }, {
                    [req.body.category.toLowerCase()]: list
                })
            } else {
                if (req.body.itemToSlot == "") {
                    await Athena.updateOne({ id: req.params.accountId }, {
                        $set: {
                            [`${req.body.category.toLowerCase()}.${req.body.slotIndex}`]: ""
                        }
                    })
                } else {
                    await Athena.updateOne({ id: req.params.accountId }, {
                        $set: {
                            [`${req.body.category.toLowerCase()}.${req.body.slotIndex}`]: `${req.body.itemToSlot.split(":")[0]}:${req.body.itemToSlot.split(":")[1].toLowerCase()}`
                        }
                    })
                }
            }
        ...
        default:
            if (req.body.itemToSlot == "") {
                ...
                await Athena.updateOne({ id: req.params.accountId }, {
                    [req.body.category.toLowerCase()]: `${req.body.itemToSlot.split(":")[0]}:${req.body.itemToSlot.split(":")[1].toLowerCase()}`
                })
            }
            break;
    }
    ...
    if (req.body.variantUpdates.length != 0) {
        await Athena.updateOne({ id: req.params.accountId }, {
            [`${req.body.category.toLowerCase()}variants`]: req.body.variantUpdates
        })
    }
